...
# =====================================================================================
# ondewo-survey-client-cpp - Makefile
#
# The ONDEWO SURVEY (Survey) gRPC client library for C++.
#
# There is no hand-written client code in this repository: everything under api/ is
# generated from the .proto files of the ondewo-survey-api submodule by the
...
LIBRARY_NAME=ondewo_survey_client
...
PACKAGE_NAME=$(LIBRARY_NAME)-$(ONDEWO_SURVEY_VERSION)-$(PACKAGE_PLATFORM)
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO SURVEY C\+\+ Client ${ONDEWO_SURVEY_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-survey-client-cpp"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-survey-client-cpp repo
...
#
# Both volumes are the repo root: the input volume must contain the ondewo-survey-api proto tree, and the
...
build_library: ## Configure, compile and install the CMake library from the generated stubs
	...
	@echo "$(BLUE)[INFO]$(NC) Building $(LIBRARY_NAME) $(ONDEWO_SURVEY_VERSION) with $(BUILD_JOBS) parallel jobs ..."
	cmake -S . -B $(BUILD_DIR) \
		-DCMAKE_BUILD_TYPE=Release \
		-DCMAKE_INSTALL_PREFIX="$(INSTALL_PREFIX)" \
		-DONDEWO_LIBRARY_NAME=$(LIBRARY_NAME) \
		-DONDEWO_LIBRARY_VERSION=$(ONDEWO_SURVEY_VERSION)
...
# Every .proto under ondewo-survey-api/ondewo must have produced a <name>.pb.h. Degrades to a labelled
...
smoke_test: ## Compile and run a tiny program against the installed CMake package
	...
	@printf '%s\n' \
		...
		'  std::puts("ondewo-survey-client-cpp smoke test OK");' \
...
# immutable by then and `spc` blocks the retry, so this has to refuse before anything is pushed.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO SURVEY C++ Client ${ONDEWO_SURVEY_VERSION}' entry"; \
...
publish_dry_run: build_package verify_package ## Everything `make publish` does except the upload - no credentials needed
	@echo "$(BLUE)[INFO]$(NC) would upload: gh release upload --repo $(GH_REPO) --clobber $(ONDEWO_SURVEY_VERSION) $(PACKAGE_ARCHIVE) $(PACKAGE_ARCHIVE).sha256"
...
# code and runtime, so a consumer has to be able to tell what this archive was compiled against.
	@{ \
		...
		echo "version:     $(ONDEWO_SURVEY_VERSION)"; \
...
upload_package: ## Attach the release archive and its checksum to the GitHub release of the current version
	...
	gh release upload --repo $(GH_REPO) --clobber "$(ONDEWO_SURVEY_VERSION)" \
