<div align="center">
  ...
  <h1 align="center">
    ONDEWO SURVEY Client C++
  </h1>
</div>

## Overview

`ondewo-survey-client-cpp` is the C++ gRPC client library for the
[ONDEWO SURVEY API](https://github.com/ondewo/ondewo-survey-api) - ONDEWO's Survey service. It is a
...
| `ondewo-survey-api/`                   | submodule - the `.proto` source of truth                                 |
...
C++ has no package registry, so every [GitHub release](https://github.com/ondewo/ondewo-survey-client-cpp/releases)
carries the built package as an asset: `ondewo_survey_client-<version>-<platform>.tar.gz`, plus a
...
archive=ondewo_survey_client-${version}-${platform}.tar.gz

gh release download "${version}" --repo ondewo/ondewo-survey-client-cpp --pattern "${archive}*"
...
```cmake
find_package(ondewo_survey_client CONFIG REQUIRED)
target_link_libraries(my_app PRIVATE ondewo::ondewo_survey_client)
```

```shell
cmake -S . -B build -DCMAKE_PREFIX_PATH=/opt/ondewo_survey_client-${version}-${platform}
```
...
set(ONDEWO_LIBRARY_NAME ondewo_survey_client CACHE STRING "" FORCE)
FetchContent_Declare(
  ondewo_survey_client
  GIT_REPOSITORY https://github.com/ondewo/ondewo-survey-client-cpp.git
  GIT_TAG        2.0.0)
FetchContent_MakeAvailable(ondewo_survey_client)
...
target_link_libraries(my_app PRIVATE ondewo_survey_client)
```
...
```shell
git clone https://github.com/ondewo/ondewo-survey-client-cpp.git
cd ondewo-survey-client-cpp
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release \
  -DONDEWO_LIBRARY_NAME=ondewo_survey_client \
...
```shell
git clone https://github.com/ondewo/ondewo-survey-client-cpp.git   ## Clone repository
cd ondewo-survey-client-cpp                                        ## Change into repo directory
...
generated header you need (`api/ondewo/survey/<file>.grpc.pb.h`) to keep compile times down.
...
int main() {
  ...
  auto channel = grpc::CreateChannel("survey.ondewo.com:443", grpc::SslCredentials({}));

  // Replace Services/Request/Response with the service you need - see the API documentation.
  auto stub = ondewo::survey::Services::NewStub(channel);

  grpc::ClientContext context;
  context.AddMetadata("authorization", "Bearer " + std::string(getenv("ONDEWO_TOKEN")));

  ondewo::survey::Request request;
  ondewo::survey::Response response;
...
docker run \
  -v $(pwd):/input-volume \
  -v $(pwd):/output-volume \
  ondewo-cpp-proto-compiler ondewo-survey-api ondewo ondewo_survey_client
```
...
- In the output volume it deletes only what it owns - `api/`, `include/ondewo_survey_client/`,
  `lib/libondewo_survey_client.a` and `lib/cmake/ondewo_survey_client/` - so a proto that was renamed or
...
- `check_build` asserts that every `.proto` under `ondewo-survey-api/ondewo` produced a matching `.pb.h`.
- `smoke_test` writes a throwaway project that does `find_package(ondewo_survey_client CONFIG REQUIRED)`,
  includes `public-api.h` and links `ondewo::ondewo_survey_client`, then compiles and runs it. That is the
