...
// MessagesInFile filters out the synthetic map-entry type protoc generates for every map<>
// field: it has no standalone generated C++ class, so the sweep below would fail to
// instantiate it. The SURVEY protos declare no map<> field at all, which leaves that filter
// unexercised in this repository - so it is pinned here against google/protobuf/struct.proto,
// which libprotobuf registers into this same generated pool (ondewo/survey/fhir.proto imports
// it) and whose google.protobuf.Struct declares exactly one map field.
//
// This is the only test in this file that is NOT in the ondewo-nlu-client-cpp original, where
// the NLU protos cover the branch on their own; it is the same trick as
// FillScalarFieldsHandlesEveryProtobufScalarType below, applied to the other product-dependent
// branch of descriptor_probe.cc.
TEST(GeneratedStubs, MessagesInFileSkipsSyntheticMapEntryTypes) {
  std::vector<std::string> names;
  for (const google::protobuf::Descriptor* descriptor :
       MessagesInFile("google/protobuf/struct.proto")) {
    names.push_back(descriptor->full_name());
  }
  ASSERT_FALSE(names.empty()) << "google/protobuf/struct.proto is not in the descriptor pool";

  EXPECT_NE(std::find(names.begin(), names.end(), "google.protobuf.Struct"), names.end())
      << "google.protobuf.Struct was dropped along with its map entry";
  EXPECT_EQ(std::find(names.begin(), names.end(), "google.protobuf.Struct.FieldsEntry"),
            names.end())
      << "the synthetic map-entry type of a map<> field was not filtered out";

  // ... and the map entry really is declared, so the filter had something to remove.
  EXPECT_NE(FindMessage("google.protobuf.Struct.FieldsEntry"), nullptr);
}
